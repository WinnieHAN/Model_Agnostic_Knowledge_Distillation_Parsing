...
import numpy as np
from dependency_parsing_tf.base_model import Model
from dependency_parsing_tf.params_init import random_uniform_initializer, random_normal_initializer, xavier_initializer
from dependency_parsing_tf.utils.general_utils import Progbar
from dependency_parsing_tf.utils.general_utils import get_minibatches
...
from word_level import glove_utils
import codecs
...
def main(flag, load_existing_dump=False):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        if flag == Flags.TRAIN:
            ...
            def select_in_matrix(word, lengths, ori_word_embedding, adv_word_embedding):
                for batch_i in range(len(word)):
                    changed_num = 0
                    for batch_j in range(0, lengths[batch_i].numpy()):
                        one_word = word[batch_i][batch_j].item()
                        # if one_word == 1:
                        #     continue
                        src_word_idx = one_word
                        adv_vector = adv_word_embedding.numpy()[src_word_idx, :]
                        ...
                        dist = a + c_ + b
                        neighbours, _ = glove_utils.pick_most_similar_words_from_vector(src_word_idx, adv_vector, dist,
                                                                                        ret_count=3, threshold=20)
                ...
                print('changed_num', changed_num)
            ori_word_embedding = model.word_embedding_matrix.eval(sess).copy()

            for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs, dataset.train_targets],
                                    config.batch_size, shuffle=False, is_multi_feature_input=True)):
                summary, loss = model.train_on_batch(sess, train_x, train_y, merged)
                adv_word_embedding = model.word_embedding_matrix.eval(sess).copy()
                lengths = 0
                word = train_x
                word_alphabet = dataset.idx2word
                new_words = select_in_matrix(word=word, lengths=lengths, ori_word_embedding=ori_word_embedding, adv_word_embedding=adv_word_embedding)

                sess.run(tf.assign(model.word_embedding_matrix, ori_word_embedding))

                for batch_i in range(len(word)):
                    wf.write(' '.join(
                        [str(word_alphabet[wordi]) for wordi in word[batch_i][1:] if not wordi == 1]))
                    wf.write('\n')
                for batch_i in range(len(word)):
                    ws = [str(word_alphabet[wordi]) for wordi in word[batch_i][1:] if not wordi == 1]
                    for w_i in range(len(ws)):
                        wf_conllu.write(str(w_i + 1) + '\t' + ws[
                            w_i] + '\t' + '_' + '\t' + 'NNP' + '\t' + 'NNP' + '\t' + '_' + '\t' + str(
                            1) + '\tnn\t_\t_\n')
                    wf_conllu.write('\n')
            wf.close()
            ...
            num_word, dim_word = model.word_embedding_matrix.eval(sess).shape
            # ----------------------------------------------
            # Testing

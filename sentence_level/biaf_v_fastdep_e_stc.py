...
from seq2seq_rl.rl import LossRL, DistLossBiafRL, get_bleu, get_correct
import pickle
...
def main():
    ...
    device = torch.device('cpu')  #torch.device('cuda:0') if args.cuda else torch.device('cpu') #TODO:8.8
    ...
    loss_biaf_rl = DistLossBiafRL(device=device, word_alphabet=word_alphabet, vocab_size=num_words).to(device)
            ...
            for epoch_i in range(EPOCHS):
                ...
                for kkk in range(1, num_batches + 1): #num_batches
                    ...
                    if True:  #inp.size()[1]<15:#True:  #inp.size()[1]<15: #TODO: debug hanwj
                        ...
                        sudo_heads_pred = fastdep_predict(test_data=test_data, dataset=dataset, sess=sess, model=model,
                                                      batch_max_length=batch[0].shape[1])
                        ls_rl_bh, reward1, reward5 = loss_biaf_rl(sel, pb, predicted_out=heads_pred, golden_out=None, mask_id=END_token,
                                                                                      stc_length_out=lengths_sel, sudo_golden_out=sudo_heads_pred, sudo_golden_out_1=None,
                ...
                for batch in conllx_data.iterate_batch_tensor(data_test, batch_size_for_eval):  # batch_size
                    ...
                    with torch.no_grad():
                        ...
                        ls_rl_bh, reward1, reward5 = loss_biaf_rl(sel, pb, predicted_out=heads_pred, golden_out=heads, mask_id=END_token, stc_length_out=lengths_sel,sudo_golden_out=sudo_heads_pred,sudo_golden_out_1=sudo_heads_pred,ori_words=word,ori_words_length=lengths)  # TODO: (sel, pb, heads)  # heads is replaced by dec_out.long().to(device)
